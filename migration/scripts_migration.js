...
migration('Script', (item, callback) => {
    async.eachSeries(item.scenarios, (scenario, scenario_callback) => {
        async.eachSeries(scenario.steps, (step, step_callback) => {
            ...
            if (step_error)
                return scenario_callback('ERROR: ' + step_error);

            scenario_callback();
        });
    }, (scenario_error) => {
        if (scenario_error)
            return callback(scenario_error);
